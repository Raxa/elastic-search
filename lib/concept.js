...
Concept.prototype.index = function(callback) {
    ...
    async.parallelLimit([
        ...
        function(callback) {
            connection.query(query_name, function(error, values, fields) {
                if (!error) {
                    async.eachLimit(values,5,function(item,callback){
                        var name = {
                            id : item.concept_id,
                        ...
                        searchserver.index(indexName, indexType, name).on('data', function(data) {
                            callback();
                        }).exec();
                    },function(err,res) {
                        callback();
            ...
            connection.query(query_set, function(error, values, fields) {
                if (!error) {
                    async.eachLimit(values,5,function(item,callback){
                        ...
                        searchserver.index(indexName, indexType, set).on('data', function(data) {
                            callback();
                        }).exec();
                    },function(err,res){
                        callback();
            ...
            connection.query(query_desc, function(error, values, fields) {
                if (!error) {
                    async.eachLimit(values,5,function(item,callback){
                        ...
                        searchserver.index(indexName, indexType, desc).on('data', function(data) {
                            callback();
                        }).exec();
                    },function(err,res){
                        callback();
    ...
    function(err){
        async.eachLimit(concepts,5,function(item,callback){
            var concept = {
                id: item.concept_id,
            ...
            // callback, when all fields are set
                ...
                searchserver.index(indexName, indexType, concept).on('data', function(data) {
                    callback();
                }).exec();    
            });
        },function(err,res){
