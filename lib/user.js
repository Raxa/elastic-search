...
User.prototype.update = function(callback) {
    ...
    setInterval(function() {
        async.filter(users,function(item,callback){
            if (item.exp < (new Date()).valueOf()) users.splice(users.indexOf(item),1);
            callback();
        },function(res){
...
User.prototype.check = function(login,pass,callback) {
    ...
    async.detect(users,function(item,callback){
        if (item.login === login) {
            // pro-long session
            users[users.indexOf(item)].exp = ((new Date()).valueOf() + config.appSessionLength * 60*1000);
            callback(true);
        }
        else callback(false);
    },function(res) {
