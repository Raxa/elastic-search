...
function Person(module) {
    ...
    searchserver = module.searchserver;
    ...
    this.type = module.objectType.person;
}
    ...
    // callback function
        ...
        async.each(persons,function(item,callback){
            ...
            async.parallel([
                // filter names
                function(callback) {
                    async.filter(names,function(item,callback){
                        if (item.person_id === person.id) callback(true);
                        else callback(false);
                    },function(results){
                        if (results.length > 0) {
                            // set person's field
                            person.data.preferredName = results[0];
                            person.data.display = results[0].givenName + ' ' + 
                                    results[0].middleName + ' ' + results[0].familyName;
                        }
                        callback();
                    });
                },
