                ...
                async.series([
                        ...
                        if (persons) {
                            async.detect(persons,function(item,callback){
                                ...
                                if (result) {
                                    ...
                                    patient.data.display = "patient " + result.given_name;
                                    if (result.middle_name) patient.data.display += (' ' + result.middle_name);
                                    patient.data.display+= (' ' + result.family_name);
                                    // fill tags
                    ...
                    function(callback) {
                        if (identifiers) {
                                ...
                                if (item.patient_id === patient.id) callback(true);
                                else callback(false);
                            },function(results){
...
Patient.prototype.search = function(data, callback) {
    ...
    var query;
    if (data === '') {
        query = new Query(objectType.person);
        query.addField('type');
    }
    else {
