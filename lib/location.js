...
Location.prototype.river = function(callback) {
    // get attributes
    var query_attribute = "SELECT location_id AS 'id', \n\
    ...
    var query_location =  "SELECT location_id AS 'id',\n\
                                  ...
                                  'location' AS 'data.display',\n\
                                  parent_location AS 'data.parentLocation',\n\
                                  uuid AS 'tags.uuid',\n\
                                  name AS 'tags.name' \n\
                             FROM location";
    // get tags
    var query_tag =       "SELECT location_id AS 'id',\n\
                                  name AS 'data.name',\n\
                                  description AS 'data.description'\n\
            ...
            async.each(value.data.hits,function(loc,callback){
                if (!options.quick) {
                    async.parallel([
                        ...
                        // get tags
                        function(callback) {
                            getDataByField(objectType.location_tag,'id',loc._source.id,function(tags){
                        ...
                        // get attributes
                        function(callback) {
                            getDataByField(objectType.location_attribute,'id',loc._source.id,function(atts){
